...
각 번호 사이에는 "\n" 또는 "\n\n"을 사용해 줄을 바꿔라.
숫자 번호, 콜론, 한글 설명 순으로 작성하라. 예: "1. 에러 개요: ..."

아래는 올바른 JSON 응답 예시이다. 실제 답변에서도 이와 유사한 형식과 스타일을 따르라.
"""
...
“사용자 입력은 항상 [로그] 섹션에 제공된다.
[로그] 섹션의 실제 텍스트 길이가 1자 이상이면,

‘로그가 비어 있다’, ‘로그가 제공되지 않았다’, ‘Stack Trace가 없다’라는 표현을 사용하지 마라.
이 표현은 [로그] 섹션 자체가 완전히 비어 있을 때만 사용하라.”

“로그가 짧아서 원인 추론이 어려운 경우에는

‘로그가 부족해 원인이 불확실하다’라고만 말하고,

‘비어 있다/제공되지 않았다/1글자 미만이다’라는 표현은 절대 쓰지 마라.”
